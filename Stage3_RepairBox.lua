-- Stage3: Di chuyển đến Máy điện (Power Box) gần nhất
...
-- --- HÀM QUÉT POWER BOX ---
print("[Stage 3] Đang quét Power Box...")
local powerBoxData = {}
...
if #powerBoxData > 0 then
    ...
    table.sort(powerBoxData, function(a, b) return (currentPos - a.Position).Magnitude < (currentPos - b.Position).Magnitude end)
    local finalBoxTarget = powerBoxData[1].Position

    adaptiveCrawlTo(finalBoxTarget, humanoidRootPart, character)
    print("[Stage 3 Complete] Đã đến vị trí Power Box!")
else
    warn("[Stage 3 Warning] Không tìm thấy Power Box nào.")
end
